class Giveaway {
    ...
    async init() {
        this.msg = await this.channel.buildEmbed()
            .setColor(0xcc8822)
            .setTitle(this.title)
            .setDescription(`ری اکشن کنید تا در گیواوی شرکت داده بشید\nتعداد برنده ها: ${this.winnerCount}\n`)
            .setFooter(`${this.suffix} | زمان اتمام`)
    ...
    async intervalFunction() {
        ...
        if(Date.now() >= this.endTime){
            ...
            if (!list.length) {
                embed.description = `برنده : No one.`;
                embed.footer.text = `گیواوی تمام شد`;
            ...
            embed.description = `ّبرنده : ${this.winners.filter(u => u !== undefined && u !== null).map(u => u.toString()).join(", ")}`;
            embed.footer.text = `${this.suffix} | گیواوی تمام شد`;

            this.msg.edit({ embed });

            if (this.winners.length) this.msg.channel.send(`تبریک , ${this.winners.map(u => u.toString()).join(", ")}! شما گیواوی را بردید **${this.title}**!`);
        }
